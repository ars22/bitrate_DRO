...
def get_model(model, pretrained, resume, n_classes, train_data, log_dir):
    if resume:
...
class Adv(torch.nn.Module):
    ...
    def __init__(self, input_dim, n_classes=2, t=1):
    ...
    def get_scaled_wts(self, X, y):
        output = torch.zeros_like(y).float() 
        for label in range(self.n_classes):
            output[y==label] = self.classifier[label](X[y==label]).squeeze() * self.t
        return torch.sigmoid(output)
    
    
def get_adv_model(model, input_dim, n_classes):
    if model == 'linear':
        model = Adv(input_dim=input_dim, n_classes=n_classes, t=10.)
    else:
        raise ValueError(f"{model} Model not recognized.")
    return model


def get_lp_norm(model, p=2):
    norm = 0.
    for param in model.parameters():
        norm += torch.norm(param.view(-1), p=p).pow(p)
    return norm
