...
from data.spurious_dataset import SpuriousDataset
...
class WaterbirdsDataset(SpuriousDataset):
    ...
    def __init__(
        ...
        if model_attributes[self.model_type]["feature_type"] == "precomputed":
            ...
            self.train_transform = get_transform_waterbirds(self.model_type,
                                                     train=True,
                                                     augment_data=augment_data)
            self.eval_transform = get_transform_waterbirds(self.model_type,
                                                    train=False,
                                                    augment_data=augment_data)


def get_transform_waterbirds(model_type, train, augment_data):
    scale = 256.0 / 224.0
